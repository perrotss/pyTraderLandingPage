...
function parseEmojiTestFile(data) {
  ...
  data.split("\n").forEach((line) => {
    ...
    results.push({
      ...
      subgroup,
      code,
      sequence: emoji_cleanup.getEmojiSequenceFromString(code),
      emoji,
      status,
      version,
      name
...
function getQualifiedEmojiSequencesMap(sequences, toString) {
  ...
  for (let i = 0; i < sequences.length; i++) {
    ...
    const unqualified = convert(emoji_cleanup.getUnqualifiedEmojiSequence(sequences[i]));
  ...
  for (const key in results) {
    ...
    map.set(
      emoji_cleanup.getEmojiSequenceFromString(key),
      emoji_cleanup.getEmojiSequenceFromString(value)
    );
  }
  return map;
}
...
exports.getQualifiedEmojiSequencesMap = getQualifiedEmojiSequencesMap;
exports.mapEmojiTestDataBySequence = mapEmojiTestDataBySequence;
