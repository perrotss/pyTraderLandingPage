...
function getResolvedPath(filename, extension, dest) {
  ...
  return require$$0$2.resolve(`${getDestPath(dest)}/${filenameWithExtension}`);
}
function writeXmlFile(resolvedPath, str, options) {
  require$$0$1.writeFileSync(resolvedPath, options.readable ? xmlFormatter(str) : str);
}
...
`).concat(`Sitemap: ${getFinalSitemapPath(options)}`);
  require$$0$1.writeFileSync(resolvedPath, str);
}
...
function generateSitemap(options) {
  ...
  if (!resolvedOptions.routes.length)
    return;
  if (!require$$0$1.existsSync(getDestPath(resolvedOptions.dest)))
    require$$0$1.mkdirSync(getDestPath(resolvedOptions.dest));
